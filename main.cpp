...
int CircleCom(int Big,int Lite) {//judge & compute the circle shade
	...
	if ((dist < R) && (dist > r)) {
		cout <<"the shade area is " <<pi * (pow(R, 2) - pow(r, 2)) << endl;
	}
	else {
		cout << 0 << endl;
	}
...
int The_min(int a,int b,int c) {//compare three numbers
